                ...
                <div class="col-12">
                    <div class="main__title">
                        ...
                        <div class="main__title-wrap">
                            ...
                            <!-- search -->
                            <form action="{{ route('admin/user/unblock/serch') }}" method="post" class="main__title-form">
                                            ...
                                            @else
                                                <form action="" method="post">
                                                    <select name="role" id="select" class="form__select"
                                                        style="margin:auto; width:80%;" onchange="javascript:location.href = this.value;"> 
                                                        @if ($value_user->role == 0)
                                                            <option selected>Người dùng</option>
                                                        @else
                                                            <option value="{{ route('admin/user/role/' . $value_user->id_user . '/0') }}">
                                                                Người dùng</option>
                                                        @endif
                                                        ...
                                                        @else
                                                            <option value="{{ route('admin/user/role/' . $value_user->id_user . '/1') }}">Admin</option>
                                                        @endif

                                                        @if ($value_user->role == 2)
                                                            <option selected>Nhân viên</option>
                                                        @else
                                                            <option value="{{ route('admin/user/role/' . $value_user->id_user . '/2') }}">Nhân viên</option>
                                                        @endif
                                                    </select>
                                                </form>
                                            @endif
                        ...
                        <ul class="paginator">
                            @if ($page > 1)
                                <li class="paginator__item paginator__item--prev">
                                    <a href="{{ route('admin/user/unblock/' . $page - 1) }}"><i
